...
def calculate_duration_between_hospitalizations(
    ...
    print(df["previous_release"])
    if df["previous_release"] is np.NaN:
        df["hospitalization_number"] = 1
    else:
        temp_hospitalization_number = df[
            (df[patient_col] == df[patient_col]) & (df[release_col] == df["previous_release"]) & (
                        df["hospitalization_number"] == df["hospitalization_number"])]
        df["hospitalization_number"] = temp_hospitalization_number + 1
