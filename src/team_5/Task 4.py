...
def replace_codes_with_names(codes_str):
    if isinstance(codes_str, str):  # Ensure the value is a string
        ...
        names = [mapping_dict.get(code, code) for code in codes]  # Use code itself if not found
        return ', '.join(names)  # Join the names back into a string
...
df_GeneralData.to_CSV(output_file_path, index=False)
